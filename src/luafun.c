// luafun.c - Lua wrapper functions for iOS port
// Adapted from jinyong-legend
...
int HAPI_LoadPic(lua_State *pL) {
    ...
    int nooffset = 0;
    int bright = 0;
    if (lua_isnoneornil(pL, 5) == 0) nooffset = (int)lua_tonumber(pL, 5);
    if (lua_isnoneornil(pL, 6) == 0) bright = (int)lua_tonumber(pL, 6);
...
int Byte_create(lua_State *pL) {
    int x = (int)lua_tonumber(pL, 1);
    char *p = (char *)lua_newuserdata(pL, x);
    int i;
    if (p == NULL) {
...
int Byte_getstr(lua_State *pL) {
    ...
    int i;
    for (i = 0; i < length; i++) s[i] = p[start + i];
    s[length] = '\0';
    lua_pushstring(pL, s);
    SafeFree(s);
    return 1;
...
int Byte_setstr(lua_State *pL) {
    ...
    int i;
    int l = (int)strlen(s);
    for (i = 0; i < length; i++) p[start + i] = 0;
    if (l > length) l = length;
    for (i = 0; i < l; i++) p[start + i] = s[i];
    lua_pushstring(pL, s);
    return 1;
}
